...
function processCodeBlocks(content: string): { html: string; codeMap: Map<string, string> } {
  const codeMap = new Map<string, string>();
  
  // This regex finds all <pre><code> blocks
  const processedHtml = content.replace(/<pre><code>([\s\S]*?)<\/code><\/pre>/g, (match, codeContent) => {
  ...
  return { html: processedHtml, codeMap };
}

export default function CodeBlock({ content }: CodeBlockProps) {
  useEffect(() => {
    // Store the code map in a data attribute on the container
    const codeMap = new Map<string, string>();
    const { html, codeMap: newCodeMap } = processCodeBlocks(content);
    const container = document.createElement('div');
    container.innerHTML = html;
    newCodeMap.forEach((code, id) => {
      codeMap.set(id, code);
    });
  ...
  const { html } = processCodeBlocks(content);
